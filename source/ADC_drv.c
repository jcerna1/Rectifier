...
void ADC_init(void)
{
    ...
    // ADC trigger setup
    ADC_MODUL4.CMPB = 0x0000;
    ADC_MODUL4.ETSEL.bit.SOCASEL = ET_CTR_ZERO;   // trigger on period
    ADC_MODUL4.ETPS.bit.SOCAPRD = ET_1ST;        // at each first case
    ADC_MODUL4.ETCLR.bit.SOCA = 1;               // clear possible flag
    ADC_MODUL4.ETSEL.bit.SOCAEN = 1;             // enable ADC Start Of conversion


    // SO0C config - dummy
    AdcRegs.ADCSOC0CTL.bit.CHSEL= 0; //input U_FAZA1
    AdcRegs.ADCSOC0CTL.bit.TRIGSEL = 0x0B;  //set SOC0 to start trigger on EPWM4A
    AdcRegs.ADCSOC0CTL.bit.ACQPS = 0X015;

    // SOC1 config
    AdcRegs.ADCSOC1CTL.bit.CHSEL= 0; //input U_FAZA1
    AdcRegs.ADCSOC1CTL.bit.TRIGSEL = 0x0B;  //set SOC0 to start trigger on EPWM4A
    AdcRegs.ADCSOC1CTL.bit.ACQPS = 0X015;

    // SOC2 config
    AdcRegs.ADCSOC2CTL.bit.CHSEL= 1; //input U_FAZA2
    AdcRegs.ADCSOC2CTL.bit.TRIGSEL = 0x0B;  //set SOC1 to start trigger on EPWM4A
    AdcRegs.ADCSOC2CTL.bit.ACQPS = 0X015;

    // SOC3 config
    AdcRegs.ADCSOC3CTL.bit.CHSEL= 2; //input U_FAZA3
    AdcRegs.ADCSOC3CTL.bit.TRIGSEL = 0x0B;  //set SOC2 to start trigger on EPWM4A
    AdcRegs.ADCSOC3CTL.bit.ACQPS = 0X015;

    // SOC4 config
    AdcRegs.ADCSOC4CTL.bit.CHSEL= 4; //input I_FAZA1
    AdcRegs.ADCSOC4CTL.bit.TRIGSEL = 0x0B;  //set SOC3 to start trigger on EPWM4A
    AdcRegs.ADCSOC4CTL.bit.ACQPS = 0X015;

    // SOC5 config
    AdcRegs.ADCSOC5CTL.bit.CHSEL= 5; //input I_FAZA2
    AdcRegs.ADCSOC5CTL.bit.TRIGSEL = 0x0B;  //set SOC4 to start trigger on EPWM4A
    AdcRegs.ADCSOC5CTL.bit.ACQPS = 0X015;

    // SOC6 config
    AdcRegs.ADCSOC6CTL.bit.CHSEL= 6; //input I_FAZA3
    AdcRegs.ADCSOC6CTL.bit.TRIGSEL = 0x0B;  //set SOC5 to start trigger on EPWM4A
    AdcRegs.ADCSOC6CTL.bit.ACQPS = 0X015;

    // SOC7 config
    AdcRegs.ADCSOC7CTL.bit.CHSEL= 8; //input U_DC
    AdcRegs.ADCSOC7CTL.bit.TRIGSEL = 0x0B;  //set SOC6 to start trigger on EPWM4A
    AdcRegs.ADCSOC7CTL.bit.ACQPS = 0X015;

    // SOC8 config
    AdcRegs.ADCSOC8CTL.bit.CHSEL= 9; //input I_DC
    AdcRegs.ADCSOC8CTL.bit.TRIGSEL = 0x0B;  //set SOC7 to start trigger on EPWM4A
    AdcRegs.ADCSOC8CTL.bit.ACQPS = 0X015;

    // SOC9 config
    AdcRegs.ADCSOC9CTL.bit.CHSEL= 12; //input ADC_POT1
    AdcRegs.ADCSOC9CTL.bit.TRIGSEL = 0x0B;  //set SOC8 to start trigger on EPWM4A
    AdcRegs.ADCSOC9CTL.bit.ACQPS = 0X015;

    // SOC10 config
    AdcRegs.ADCSOC10CTL.bit.CHSEL= 14; //input ADC_POT2
    AdcRegs.ADCSOC10CTL.bit.TRIGSEL = 0x0B;  //set SOC9 to start trigger on EPWM4A
